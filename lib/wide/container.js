...
// a set of `wide.Logger` instances based on string ids.
//
...
// Retreives a `wide.Logger` instance for the specified `id`. If
...
Container.prototype.get = Container.prototype.add = function (id, options) {
    ...
    if (!this.loggers[id]) {
        ...
        // instantiated prototypal objects
        //
        ...
        Object.keys(options).forEach(function (key) {
            ...
            if (!wide.transports[name]) {
                throw new Error('Cannot add unknown transport: ' + name);
            }

            var namedOptions = options[key];
            namedOptions.id = id;
            options.transports.push(new (wide.transports[name])(namedOptions));
        });

        this.loggers[id] = new wide.Logger(options);
    }
