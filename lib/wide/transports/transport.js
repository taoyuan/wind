...
// base functionality for all wide transports.
//
...
//
// ### function logException (msg, data, callback)
// #### @msg {string} Message to log
// #### @data {Object} **Optional** Additional metadata to attach
// #### @callback {function} Continuation to respond to when complete.
// Logs the specified `msg`, `data` and responds to the callback once the log
// operation is complete to ensure that the event loop will not exit before
// all logging has completed.
//
Transport.prototype.logException = function (msg, data, callback) {
    var self = this;
    ...
    this.log({level: 'error', message: msg, data: data}, function () {
    });
};
...
Transport.prototype.log = function (options) {
    var timestampFn = typeof options.timestamp === 'function'
        ...
        data = options.data !== undefined || options.data !== null ? exports.clone(cycle.decycle(options.data)) : null,
        output;
