    ...
    function emit(name, next) {
        ...
        if ((transport.level && self.levels[transport.level] <= self.levels[level])
            ...
            transport.log(rec, function (err) {
                ...
                self.emit('logging', transport, rec);
                next();
            });
        } else {
            next();
...
function mkRecord(level, id, msg, data) {
    ...
    //
    //rec.__defineGetter__('message', function () {
    //    return this.msg;
    //});
...
//
Logger.prototype.add = function (type, options) {
    //var instance = created ? transport : (new (transport)(options));
    var instance = transports.get(type, options);

    if (!instance.name && !instance.log) {
        throw new Error('Unknown transport with no log() method');
    }
    if (this.transports[instance.name]) {
        console.warn('Override the transport already attached: ' + instance.name);
    }
