/*
 * container.js: Inversion of control container for firelog logger instances
 *
...
// a set of `firelog.Logger` instances based on string ids.
//
...
// Retreives a `firelog.Logger` instance for the specified `id`. If
...
Container.prototype.get = Container.prototype.add = function (id, options) {
  ...
  if (!this.loggers[id]) {
    ...
    Object.keys(options).forEach(function (key) {
      ...
      if (!firelog.transports[name]) {
        throw new Error('Cannot add unknown transport: ' + name);
      }

      var namedOptions = options[key];
      namedOptions.id = id;
      options.transports.push(new (firelog.transports[name])(namedOptions));
    });

    this.loggers[id] = new firelog.Logger(options);
  }
